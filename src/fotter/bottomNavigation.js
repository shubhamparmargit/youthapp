...
import { imgPath } from '../components/common/common.function';
...
function BottomNav() {
  ...
  //   // Iterate through chatContacts to populate uniqueConversations
  //   chatContacts.forEach(chat => {
  //     // Create a unique key using both the username and the isSecret flag
  //     const key = (chat.sender === user.username ? chat.receiver : chat.sender) + (chat.isSecret === "true" ? "_secret" : "_normal");

  //     // Check if the conversation is already present or if the new chat is more recent
  ...
  //   // Combine the unique conversations into an array and sort them by timestamp
  //   const sortedChats = Object.values(uniqueConversations).sort(
  //     (a, b) => new Date(b.timestamp) - new Date(a.timestamp)
  //   );

  //   // Set state for combined chats (normal + secret)
  ...
  useEffect(() => {
    ...
    chatContacts.forEach(chat => {
      // Create a unique key based on sender, receiver, isSecret, and hideMe
      const key = `${chat.sender === user.username ? chat.receiver : chat.sender}-${chat.isSecret}-${chat.hideMe}`;

      // Only add or update the conversation if it doesn't exist or the current chat is newer
      if (!uniqueConversations[key] || new Date(chat.timestamp) > new Date(uniqueConversations[key].timestamp)) {
        uniqueConversations[key] = chat;
      }
    });

    // Convert the unique conversations object into a sorted array (by time)
    const sortedChats = Object.values(uniqueConversations).sort(
      (a, b) => new Date(b.timestamp) - new Date(a.timestamp)
    );
    ...
    <>
      ...
      <div className="bottom-nav" style={{ backgroundColor: '#181818' }}>
        <div
          ...
          onClick={(e) => { handleTabChange('home'); navigate('/') }}
        >
        ...
        <div
          ...
          onClick={(e) => {
            if (token !== '') {
              handleTabChange('questions'); navigate('/myquestions')
            } else {
              navigate('/login');
            }
          }}
        ...
        <div
          ...
          onClick={() => { handleTabChange('ask'); navigate(token !== '' ? '/postquestion' : '/login', { state: myInfo }) }}
        >
        ...
        <div
          ...
          onClick={(e) => {
            if (token !== '') {
              handleTabChange('forYou'); navigate('/foryou')
            } else {
              navigate('/login');
            }
          }}
      ...
      <Modal show={show} fullscreen={true} onHide={() => setShow(false)}>
        <Row className='pt-1 defaultPurpleBg pb-2'>
          ...
          <Col lg={5} md={5} sm={5} xs={5} className='text-end px-3 mt-1'><img
            ...
            onClick={() => navigate(`/profile/${user._id}`, { state: user })} /></Col>
        </Row>
                    ...
                    <Card.Body className='p-0'>
                      <Row className='px-2 bgTheme'>
                          ...
                          <img
                            src={(chat.isSecret == "true" && user.username !== chat.hideMe ? (otherUser.gender === 'male' ? BoyD : GirlD)
                              : (otherUser.profileImg && otherUser.profileImg !== '' && otherUser.profileImg !== 'undefined'
                                ...
                                :(otherUser.gender === 'male' ? BoyD : GirlD) // Fallback for gender-based image
                              )
                            )}
                            alt=""
                        ...
                        <Col lg={9} md={9} sm={9} xs={9}
                          onClick={() => {
                            navigate("/chat", {
                              state: { ldata: otherUser, isSecret: chat.isSecret, hideMe: chat.hideMe, myInfo: myInfo }
                            });
